...
def extract_model_features(params, client_id, round_num, reference_params=None):
    ...
    if isinstance(params, dict):
        ...
        weight_layers = [name for name in all_keys if any(p in name.lower() for p in ['weight', 'kernel', 'w.', 'linear'])]
        
        if not weight_layers:
            weight_layers = all_keys
        ...
        if weight_layers:
            ...
            if len(weight_layers) > 1:
                ...
                features["first_vs_last_mean_ratio"] = np.mean(np.abs(first_layer_params)) / (np.mean(np.abs(last_layer_params)) + 1e-8)
                features["first_vs_last_std_ratio"] = np.std(first_layer_params) / (np.std(last_layer_params) + 1e-8)
    
    # Distance metrics (if reference available)
